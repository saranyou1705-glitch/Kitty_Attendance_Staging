...
function dailyTable(rows){return table(['พนักงาน','วันที่','เข้างาน','ออกงาน','ชั่วโมงสุทธิ'],(rows||[]).map(r=>[person(r.employee),r.work_date,time(r.first_in_at),time(r.last_out_at),hours(r.net_hours)]))}
...
async function calendarView(){
 ...
 return `<p>${esc(person(state.boot.employee))}</p><div class="personal-calendar-layout">${panel(calendarGrid(month.rows))}${panel(`<h2>${displayDate(state.selected)}</h2>${eventsTable(day.events)}<p>ชั่วโมงสุทธิ: ${hours(day.daily?.net_hours)}</p>`)}</div>`;
}
