...
async function dashboardView(){const d=await api('admin_daily',{date:state.date});return `${dayPicker()}<div class="stats">${[['เข้างานแล้ว',d.summary.checked_in],['ออกงานแล้ว',d.summary.checked_out],['ยังไม่เข้างาน',d.summary.not_checked_in],['ลา',d.summary.leave]].map(([label,count])=>`<article class="stat-card"><span>${label}</span><strong>${count??'—'}</strong></article>`).join('')}</div><div class="dashboard-layout"><section class="panel dashboard-main"><h2>${state.role==='hr'?'การลงเวลา · Head Office':'การลงเวลาวันนี้'}</h2>${dailyTable(d.rows)}</section><aside class="dashboard-aside"><section class="panel dashboard-report"><h2>รายงาน</h2><button class="btn primary" data-page="reports">เปิดศูนย์รายงาน</button></section><section class="panel"><h2>จัดการ</h2><div class="dashboard-links"><button class="btn secondary" data-page="employees">พนักงาน</button><button class="btn secondary" data-page="schedule">ตารางงาน</button><button class="btn secondary" data-page="leave">การลา</button><button class="btn secondary" data-page="clock-approvals">คำขอลงเวลา</button>${state.role==='admin'?'<button class="btn secondary" data-page="line">LINE Report</button>':''}</div></section></aside></div>`}
