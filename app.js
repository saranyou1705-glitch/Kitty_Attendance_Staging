...
function navigation(){
 ...
 const desktop=list.length>5?[list.find(x=>x[0]==='dashboard'),list.find(x=>x[0]==='employees'),list.find(x=>x[0]==='schedule'),list.find(x=>x[0]==='clock-approvals'),list.find(x=>x[0]==='reports'),['more','•••','จัดการ']]:list;
...
async function dashboardView(){
 ...
 return `<div class="dashboard-heading"><div><p class="page-context">${state.role==='hr'?'HR · Head Office':'Admin · ทุกกลุ่มพนักงาน'}</p><h1>ภาพรวมวันนี้</h1></div>${dayPicker()}</div><div class="stats">${[['เข้างานแล้ว',d.summary.checked_in],['ยังไม่เข้างาน',d.summary.not_checked_in],['ลา',d.summary.leave],['ออกงานแล้ว',d.summary.checked_out]].map(([label,count])=>`<article class="stat-card"><span>${label}</span><strong>${count??'—'}</strong></article>`).join('')}</div><div class="dashboard-layout"><section class="panel dashboard-main"><div class="section-heading"><h2>การลงเวลาวันนี้</h2><button class="btn secondary" data-page="employees">${uiIcon('arrow')}ดูพนักงานทั้งหมด</button></div>${attendancePeople(d.rows)}<details class="event-details"><summary>ตารางเวลารายละเอียด</summary>${dailyTable(d.rows)}</details></section><aside class="dashboard-aside"><section class="panel dashboard-report"><h2>รายงาน</h2><button class="btn primary" data-page="reports">${uiIcon('reports')}เปิดศูนย์รายงาน</button></section><section class="panel"><h2>สัปดาห์นี้</h2>${weekPicker()}<p class="week-caption">${displayDate(state.date)}</p><button class="btn secondary" data-page="schedule">ดูตารางงาน</button></section></aside><section class="panel dashboard-requests"><div class="section-heading"><h2>คำขอที่ต้องตรวจ</h2><button class="btn secondary" data-page="clock-approvals">ดูคำขอ</button></div><p class="panel-sub">ยังไม่เชื่อมข้อมูลคำขอใน Staging</p><div class="request-shortcuts"><button class="btn secondary" data-page="leave">การลา</button><button class="btn secondary" data-page="clock-approvals">คำขอลงเวลา</button>${state.role==='admin'?'<button class="btn secondary" data-page="line">LINE Report</button>':''}</div></section></div>`;
}
...
async function scheduleView(){
 const [d,attendance]=await Promise.all([api('staging_schedule',{date:state.date}),api('admin_daily',{date:state.date})]);
 return `<div class="schedule-toolbar">${dayPicker()}<button class="btn secondary" data-today>วันนี้</button></div><div class="schedule-layout">${panel(calendarGrid([],true))}${panel(`<h2>${displayDate(state.date)}</h2><p class="panel-sub">ตาราง ${d.rows.length} คน · มีเวลาเข้า ${attendance.rows.filter(r=>r.first_in_at).length} คน</p>${d.rows.map(r=>{const a=attendance.rows.find(a=>a.employee_id===r.employee_id)||{};return `<button class="schedule-entry" data-employee="${esc(r.employee_id)}"><strong>${esc(person(r.employee))}</strong><span>${esc(scheduleLabel(r.schedule_status))} · ${esc(r.office?.name||'ไม่ระบุสำนักงาน')}</span><span>กำหนด ${hours(r.required_hours)}</span><span>เข้า ${time(a.first_in_at)} · ออก ${time(a.last_out_at)}</span><span>ทำงาน ${hours(a.paid_work_hours)}</span><span class="entry-link">ดูรายละเอียด</span></button>`}).join('')||empty('ไม่พบตารางงานในวันที่เลือก')}${disabled('แก้ไขตาราง')}${disabled('สร้างเดือนหน้า')}`)}</div>`;
}
function calendarGrid(rows,management=false){
 ...
 return `<div class="month-nav"><button class="icon-button" data-shift="-1" aria-label="เดือนก่อนหน้า">‹</button><input type="month" aria-label="เดือนปฏิทิน" id="month" value="${state.month}"><button class="icon-button" data-shift="1" aria-label="เดือนถัดไป">›</button></div><div class="calendar">${['อา','จ','อ','พ','พฤ','ศ','ส'].map(x=>`<span class="cal-head">${x}</span>`).join('')}${cells.join('')}</div>${management?'<p class="calendar-key">เลือกวันเพื่อดูตารางและเวลาจริงของพนักงาน</p>':'<p class="calendar-key">แสดงเวลาเข้า หรือสถานะตาราง · กดวันที่ดูรายละเอียด</p>'}`;
}
