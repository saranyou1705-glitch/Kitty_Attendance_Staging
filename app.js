...
async function dashboardView(){
 ...
 return `<div class="dashboard-heading"><div><p class="page-context">${state.role==='hr'?'HR · Head Office':'Admin · ทุกกลุ่มพนักงาน'}</p><h1>ภาพรวมวันนี้</h1></div>${dayPicker()}</div><div class="stats">${[['เข้างานแล้ว',d.summary.checked_in],['ยังไม่เข้างาน',d.summary.not_checked_in],['ลา',d.summary.leave],['ออกงานแล้ว',d.summary.checked_out]].map(([label,count])=>`<article class="stat-card"><span>${label}</span><strong>${count??'—'}</strong></article>`).join('')}</div><div class="dashboard-layout"><section class="panel dashboard-main"><div class="section-heading"><h2>การลงเวลาวันนี้</h2><button class="btn secondary" data-page="employees">ดูพนักงานทั้งหมด ↗</button></div>${attendancePeople(d.rows)}<details class="event-details"><summary>ตารางเวลารายละเอียด</summary>${dailyTable(d.rows)}</details></section><aside class="dashboard-aside"><section class="panel dashboard-report"><h2>รายงาน</h2><button class="btn primary" data-page="reports">เปิดศูนย์รายงาน</button></section><section class="panel"><h2>สัปดาห์นี้</h2>${weekPicker()}<p class="week-caption">${displayDate(state.date)}</p><button class="btn secondary" data-page="schedule">ดูตารางงาน</button></section></aside><section class="panel dashboard-requests"><div class="section-heading"><h2>คำขอที่ต้องตรวจ</h2><button class="btn secondary" data-page="clock-approvals">ดูคำขอ</button></div><p class="panel-sub">ยังไม่เชื่อมข้อมูลคำขอใน Staging</p><div class="request-shortcuts"><button class="btn secondary" data-page="leave">การลา</button><button class="btn secondary" data-page="clock-approvals">คำขอลงเวลา</button>${state.role==='admin'?'<button class="btn secondary" data-page="line">LINE Report</button>':''}</div></section></div>`;
}
