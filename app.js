...
async function loadOvertimeBalance(button){
 ...
 try{
  ...
  if(current()){
   ...
   if(existing){$('#otBalance').innerHTML='<p class="ot-empty">'+(existing.status==='APPROVED'?'คำขอนี้อนุมัติแล้ว ดูได้ในประวัติคำขอ':'ส่งคำขอนี้แล้ว กำลังรออนุมัติ')+'</p>';return}
  }
