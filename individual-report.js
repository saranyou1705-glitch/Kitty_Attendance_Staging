...
(function(root){
 ...
 function build(data,ExcelJS){
  ...
  const sheet=wb.addWorksheet('ลงเวลารายบุคคล',{views:[{state:'frozen',xSplit:1,ySplit:7,showGridLines:false}],pageSetup:{paperSize:9,orientation:'landscape',fitToPage:true,fitToWidth:1,fitToHeight:0,printTitlesRow:'7:7'}});
  sheet.columns=[14,22,13,13,13,13,23,23,23,23,100].map(width=>({width}));
  sheet.getCell('A2').value='รายงานลงเวลารายบุคคล';sheet.getCell('A2').font={name:'Arial',size:16,bold:true};
  sheet.getCell('A3').value=`${data.employee.employee_code} · ${data.employee.name}`;
  ...
  sheet.getCell('K5').value=(data.warnings||[]).join('\n')||'หมายเหตุคำขอแสดงทั้งวันที่ส่งและวันที่เกี่ยวข้อง ไม่นับซ้ำเป็นคำขอใหม่';
  sheet.getCell('K5').alignment={wrapText:true,vertical:'top'};sheet.getRow(5).height=46;
  if(data.warnings?.length)sheet.getCell('K5').font={name:'Arial',size:11,color:{argb:'FF9C6500'}};
  const headers=['วันที่','สถานะตาราง','เข้างาน','ออกพัก','กลับจากพัก','ออกงาน','ทำงานสุทธิ','ขาด','เกิน','เวลาชด','หมายเหตุ / คำขอ'];
  ...
  data.rows.forEach((r,i)=>{
   ...
   row.values=[new Date(r.work_date+'T00:00:00Z'),status[r.schedule_status]||r.schedule_status||'',...['first_in_at','break_out_at','break_in_at','last_out_at'].map(k=>excelTime(r[k])),...['paid_work_hours','short_hours','over_hours','makeup_hours'].map(k=>excelDuration(r[k])),text||null];
   ...
   for(let c=3;c<=6;c++)row.getCell(c).numFmt='hh:mm';
   for(let c=7;c<=10;c++)row.getCell(c).numFmt='[h]" ชม. "mm" นาที"';
   row.getCell(11).alignment={wrapText:true,vertical:'top'};
   if(flagged(r))row.getCell(11).font={name:'Arial',size:11,bold:true,color:{argb:'FFB91C1C'}};
   row.height=Math.min(409,Math.max(26,text.split('\n').reduce((sum,line)=>sum+Math.max(1,Math.ceil(line.length/80)),0)*17+8));
  });
  sheet.autoFilter={from:'A7',to:`K${7+data.rows.length}`};
  const n=data.rows.length+8;sheet.getCell(`B${n}`).value='รวมเวลาที่มีข้อมูล';
  for(let c=7;c<=10;c++){
   const col=String.fromCharCode(64+c);const values=data.rows.map(r=>excelDuration(r[['paid_work_hours','short_hours','over_hours','makeup_hours'][c-7]])).filter(v=>v!==null);
  ...
  sheet.getCell(`K${n}`).value='ช่องว่าง = ไม่มีข้อมูล ไม่ใช่ 0 ชั่วโมง';
  return wb;
 }
